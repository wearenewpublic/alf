  ...
  return {
    async get(sub: string): Promise<NodeSavedSession | undefined> {
      ...
      try {
        ...
        const dpopBlob = JSON.parse(decrypt(row.dpop_private_key, encryptionKey)) as
          | { jwk: Record<string, unknown>; aud: string }
          | Record<string, unknown>;

        // New format: { jwk, aud }. Legacy format: bare JWK object.
    ...
    async set(sub: string, value: NodeSavedSession): Promise<void> {
      ...
      const encryptedDpopKey = encrypt(
        JSON.stringify({ jwk: dpopJwk, aud: typeof tokenSet.aud === 'string' ? tokenSet.aud : '' }),
        encryptionKey,
      );
